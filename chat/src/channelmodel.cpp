...
FriendNode::FriendNode(Node *parent, chat::Friend *f) : Node(NodeType::LegacyFriend, parent), f(f) {
    connect(f, SIGNAL(connection_changed(Link, Link)), this, SLOT(connection_changed(Link, Link)));
    connect(f, SIGNAL(message(bool,QByteArray)), this, SLOT(message(bool,QByteArray)));
}

void FriendNode::connection_changed(chat::Link old_state, chat::Link new_state) {
    emit changed(this);
}
