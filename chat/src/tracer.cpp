...
Tracer::Tracer(chat::Core* core) {
    ...
    connect(core, SIGNAL(onLosslessPacket(uint32_t, QByteArray)), this,
            SLOT(onLosslessPacket(uint32_t, QByteArray)));
    connect(core, SIGNAL(onLossyPacket(uint32_t, QByteArray)), this,
            SLOT(onLossyPacket(uint32_t, QByteArray)));
}
