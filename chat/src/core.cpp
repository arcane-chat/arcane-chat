...
static void MyFriendTyping(Tox *tox, uint32_t friend_number, bool is_typing, void *user_data) {
    qDebug() << friend_number << is_typing << "is typing";
}

static void FriendConnectionUpdate(Tox *tox, uint32_t friend_number,
...
Core::Core(Tox *tox) : tox(tox) {
    ...
    for (int i=0; i<count; i++) {
        ...
        TOX_CONNECTION link = tox_friend_get_connection_status(tox, friends[i], nullptr);

        chat::Link newlink;

        switch (link) {
        case TOX_CONNECTION_NONE:
            newlink = chat::Link::None;
            break;
        case TOX_CONNECTION_TCP:
            newlink = chat::Link::Tcp;
            break;
        case TOX_CONNECTION_UDP:
            newlink = chat::Link::Udp;
            break;
        }

        Friend *f = new Friend(friends[i], QByteArray((const char*)pubkey, TOX_PUBLIC_KEY_SIZE), QString(QByteArray((const char*)name, size)), newlink);
...
void Core::handle_friend_connection_update(uint32_t friend_number, TOX_CONNECTION link) {
    chat::Link newlink;
    ...
    foreach (chat::Friend *f, friends) {
        if (f->friend_number == friend_number) f->set_connection(newlink);
    }
}
...
void Core::send_message(uint32_t friend_number, bool action, QString message) {
    ...
    tox_friend_send_message(tox, friend_number, TOX_MESSAGE_TYPE_NORMAL, msg, size, &error);
}
